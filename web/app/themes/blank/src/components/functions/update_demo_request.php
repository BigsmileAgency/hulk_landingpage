...
function update_demo_request()
{
	...
	$update = $wpdb->query(
		$wpdb->prepare(
			"UPDATE `wp_demo_appointement` 
      SET `first_name`='%s',`last_name`='%s',`company`='%s',`is_agency`='%s',`email`='%s',`phone`='%s',`lang`='%s',`date`='%s',`time_slot_id`='%s' 
			WHERE id = %s",
				$first_name,
				$last_name,
				$company,
...
add_action('wp_ajax_update_demo_request', 'update_demo_request');
add_action('wp_ajax_nopriv_update_demo_request', 'update_demo_request');
